...
### 3.2 📝 雙向動態勘誤引擎 (`correction_lmj.xlsx` & `correction_漢字.xlsx`)
- **白話字勘誤 (`correction_lmj.xlsx`)**：維護 `漢字 ⇋ 羅馬字`（如 `梅毒 ⇋ mûi-to̍k`），自動校準正文中因百年鉛字印刷墨漬破損導致之拼寫變異，並於辭典中自動合併累計頻率。
- **台語漢字勘誤 (`correction_漢字.xlsx`)**：維護 `錯誤漢字 ⇋ 正確漢字`（如 `落用 ⇋ 路用`、`個 ⇋ 个`、`見識 ⇋ 見色`），自動修正全書對照引言框、內文註解、三語辭彙與總索引中的台語用字。
