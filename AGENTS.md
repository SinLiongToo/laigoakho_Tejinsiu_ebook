...
## 📝 3. 動態勘誤規範 (`correction_lmj.xlsx` 與 `correction_漢字.xlsx`)

### 3.1 羅馬字動態勘誤 (`correction_lmj.xlsx`)
- **勘誤表維護**：
  - 專案根目錄之 `correction_lmj.xlsx` 為持續擴充之權威白話字勘誤表。
...
### 3.2 漢字動態勘誤 (`correction_漢字.xlsx`)
- **勘誤表維護**：
  - 專案根目錄之 `correction_漢字.xlsx` 為台語漢字對照文本持續擴充之權威勘誤表。
  - 欄位結構：`錯誤漢字` ⇋ `正確漢字`（例如 `落用` ⇋ `路用`、`個` ⇋ `个`、`見識` ⇋ `見色`）。
- **校正處理原則**：
  - 由 `src/build_book.py` 內的 `load_hanzi_corrections()` 自動載入，依詞長遞減排序以避免子字串搶先替換。
  - **電子書章節與對照框**：自動校準正文與全漢對照區塊（`> **【全漢對照】**`）中之錯誤用字與非台語常用字。
  - **三語語彙與總索引**：自動同步校準辭典條目中對應之漢字標註。
